...
import QC_StrategyBacktest_init

class StrategyBacktest(QCAlgorithm):

   def initialize(self):
         return QC_StrategyBacktest_init.initialize(self)
   ...
   def OnEndOfAlgorithm(self):
      ...
      self.Log(dfAllPositions.head(0).to_csv(index = False, header = True, lineterminator = " "))
      ...
      for i in range(0, len(dfAllPositions.index)):
         self.Log(dfAllPositions.iloc[[i]].to_csv(index = False, header = False, lineterminator = " "))
      self.Log("")
      ...
      if self.positionTracking:
         ...
         self.Log("")
         # Print the data frame to the log in csv format (one row at the time to avoid QC truncation limitation)
         self.Log(dfTracking.head(0).to_csv(index = False, header = True, lineterminator = " "))
         for i in range(0, len(dfTracking.index)):
            self.Log(dfTracking.iloc[[i]].to_csv(index = False, header = False, lineterminator = " "))
         self.Log("")
